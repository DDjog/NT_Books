...
if operation_status == OPER_UPDATE_SUCCEEDED:
    print(f'Updated publisher: {updated_publisher_name} with publication year {updated_publication_year} is in the database')
elif operation_status == OPER_UPDATE_FAILED_DATA_NOT_FOUND:
    print(f'Publisher: {updated_publisher_name} with publication year {updated_publication_year} is not in the database')